function createSvgElement(textWidth, textHeight, padding) {
  ...
  svg.setAttribute(
    "viewBox",
    `0 ${-(textHeight + padding)} ${textWidth} ${textHeight}`
  );
  return svg;
}
...
function applyOutlineAnimation(svgPath, length, animationDuration, nthElem) {
  ...
  svgPath.style.animation = `line-anim ${animationDuration}s ${
    animationDuration * 0.45 * nthElem
  }s ease forwards`;
}
...
function applyFillAnimation(svgPathFill, length, animationDuration, nthElem) {
  ...
  svgPathFill.style.animation = `fill-anim ${animationDuration}s ${
    animationDuration * 0.45 * nthElem
  }s forwards`;
}
...
export function appendAnimationFromPathData(
  svg,
  pathData,
  animationDuration,
  nthElem
...
export async function makeSvgElementWithTextDrawingAnimation(
  ...
  const textHeight =
    (fontSize * (font.ascender - font.descender)) / font.unitsPerEm;
  ...
  const charPathDataArr = text
    .split("")
    .map((c) => font.getPath(`${c}`, 0, baseline, fontSize).toPathData());
  ...
  /*
    ...
    */
  const charPathDataFromStrPathData = splitPathData(
    strPathData,
    charPathCmdCntArr
  );
