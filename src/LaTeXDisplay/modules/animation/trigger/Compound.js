...
export default class CompoundTrigger extends BaseTrigger {
  ...
  #handleUnhandledKey = (event) => {
    ...
    if (keyEvent.code === "Space") {
      const targetTrigger = this.#isTimerMode
        ? this.#autoTimerTrigger
        : this.#keyboardTrigger;
      const stopOpts = this.#isTimerMode
        ? null
        : { removeKeydownHandler: false };
  ...
  performAction(initDelay) {
    const targetTrigger = this.#isTimerMode
      ? this.#autoTimerTrigger
      : this.#keyboardTrigger;
    targetTrigger.wait(initDelay).then(this.resolveWait).catch(this.rejectWait);
  }
