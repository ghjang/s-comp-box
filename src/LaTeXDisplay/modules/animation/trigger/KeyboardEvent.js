...
class BidirectionalStrategy extends DirectionStrategy {
  getDirection(event) {
    ...
    if (event.code === "ArrowRight" || event.code === "ArrowLeft") {
      selectedDirection =
        event.code === "ArrowRight" ? Direction.RIGHT : Direction.LEFT;
...
export class KeyboardTrigger extends BaseTrigger {
  ...
  performAction(_initDelay) {
    ...
    this.#onKeydown = (event) => {
      ...
      if (selectedDirection && this.resolveWait != null) {
        this.#removeKeydownHandler();
        this.resolveWait({ nextDirection: selectedDirection });
      } else {
        const customEvent = new CustomEvent("unhandledKey", {
          detail: { event, trigger: this },
        });
  ...
  stop(opts) {
    if (opts == null || opts.removeKeydownHandler) {
      this.#removeKeydownHandler();
    }
...
export function createKeyboardTrigger(strategyType) {
  ...
  switch (strategyType) {
    ...
    case "bidirectional":
      ...
      break;
    case "composite":
      strategy = new CompositeDirectionStrategy()
