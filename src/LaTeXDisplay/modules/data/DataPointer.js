...
export default class DataPointer {
  ...
  get curPosData() {
    return this.pointerX >= 0 && this.pointerX < this.data.length
      ? this.data[this.pointerX]
      : null;
  }
  ...
  async #moveToEnd() {
    while (
      this.pointerX >= 0 &&
      this.data[this.pointerX].value.endOfRow == null
    ) {
      if (this.pointerX < this.data.length - 1) {
        ++this.pointerX;
      } else if (this.iterator) {
  ...
  #moveToHome() {
    while (
      this.pointerX > 0 &&
      this.data[this.pointerX].value.startOfRow == null
    ) {
      --this.pointerX;
    }
  }
  ...
  async #moveToCtrlEnd() {
    while (
      this.pointerX >= 0 &&
      this.data[this.pointerX].value.endOfExpressions == null
    ) {
      if (this.pointerX < this.data.length - 1) {
        ++this.pointerX;
      } else if (this.iterator) {
    ...
    for (const nextDirection of nextDirections) {
      switch (nextDirection) {
        ...
        case Direction.CTRL_HOME:
          await this.#moveToCtrlHome();
          break;
        case Direction.CTRL_END:
          await this.#moveToCtrlEnd();
          break;
