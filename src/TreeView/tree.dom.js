export function getFirstNodeId(rootUlElem) {
    ...
    const firstNodeElem = rootUlElem.querySelector(
...
export function getNextNodeId(rootUlElem, nodeId) {
    ...
    if (isNodeOpen) {
        ...
        if (childUl) {
            nextNodeId = childUl.querySelector(".node-content[data-node-id]")
                ?.dataset.nodeId;
        }
    }
    ...
    if (!nextNodeId) {
        ...
        if (nextLi) {
            nextNodeId = nextLi.querySelector(".node-content[data-node-id]")
                ?.dataset.nodeId;
        }
    }
